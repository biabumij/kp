	...
	{   
       ...
       if($query->num_rows() > 0){
			foreach ($query->result_array() as $key => $row) {
				...
				$row['print'] = '<a href="'.site_url().'evaluasi/cetak_evaluasi_supplier/'.$row['id'].'" target="_blank" class="btn btn-info" style="font-weight:bold; border-radius:10px;"><i class="fa fa-print"></i> </a>';
				...
				if($this->session->userdata('admin_group_id') == 1 || $this->session->userdata('admin_group_id') == 4 || $this->session->userdata('admin_group_id') == 5 || $this->session->userdata('admin_group_id') == 2){
					$row['actions'] = '<a href="javascript:void(0);" onclick="DeleteEvaluasiSupplier('.$row['id'].')" class="btn btn-danger" style="font-weight:bold; border-radius:10px;"><i class="fa fa-close"></i> </a>';
				}else {
