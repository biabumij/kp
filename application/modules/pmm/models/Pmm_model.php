...
class Pmm_model extends CI_Model {
   ...
   function GetSPOByWeek($schedule_id,$week)
   {
    ...
    function TableMainPengadaan($id)
    {
        ...
        if($query->num_rows() > 0){
            foreach ($query->result_array() as $key => $row) {
				...
				$row['total']= number_format($row['total'],0,',','.');
                $row['actions'] = '<a href="javascript:void(0);" onclick="OpenFormMain('.$row['id'].')" class="btn btn-success" style="font-weight:bold; width:200px; border-radius:10px;">Update Pengadaan </a>';
    ...
    //RUMUS BARU
    function GetPenerimaanPembelianTEMEF($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false,$filter_kategori=false)
    {
        $output = array();

        $this->db2->select('ppo.supplier_id, prm.purchase_order_id, prm.display_measure as measure,p.nama_produk,prm.material_id,SUM(prm.display_price) / SUM(prm.display_volume) as price,SUM(prm.display_volume) as volume, SUM(prm.display_price) as total_price');
        $this->db2->join('produk p','prm.material_id = p.id','left');
        ...
        if(!empty($purchase_order_no)){
            $this->db2->where('ppo.id',$purchase_order_no);
        }
        if(!empty($filter_material)){
            $this->db2->where_in('prm.material_id',$filter_material);
        }
        if(!empty($filter_kategori)){
            $this->db2->where_in('ppo.kategori_id',$filter_kategori);
        }
		$this->db2->where("ppo.status in ('PUBLISH','CLOSED')");
        $this->db2->order_by('p.nama_produk','asc');
        $this->db2->group_by('prm.material_id');
    ...
    function GetPenerimaanPembelianTEMEFPrint($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false,$filter_kategori=false)
    {
        $output = array();

        $this->db2->select('prm.purchase_order_id, prm.display_measure as measure,p.nama_produk,prm.material_id,SUM(prm.display_price) / SUM(prm.display_volume) as price,SUM(prm.display_volume) as volume, SUM(prm.display_price) as total_price');
        $this->db2->join('produk p','prm.material_id = p.id','left');
        $this->db2->join('pmm_purchase_order ppo','prm.purchase_order_id = ppo.id','left');
        if(!empty($start_date) && !empty($end_date)){
            $this->db2->where('prm.date_receipt >=',$start_date);
    ...
    function GetPenerimaanPembelianSC($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false,$filter_kategori=false)
    {
        $output = array();

        $this->db3->select('ppo.supplier_id, prm.purchase_order_id, prm.display_measure as measure,p.nama_produk,prm.material_id,SUM(prm.display_price) / SUM(prm.display_volume) as price,SUM(prm.display_volume) as volume, SUM(prm.display_price) as total_price');
        $this->db3->join('produk p','prm.material_id = p.id','left');
        ...
        if(!empty($purchase_order_no)){
            $this->db3->where('ppo.id',$purchase_order_no);
        }
        if(!empty($filter_material)){
            $this->db3->where_in('prm.material_id',$filter_material);
        }
        if(!empty($filter_kategori)){
            $this->db3->where_in('ppo.kategori_id',$filter_kategori);
        }
		$this->db3->where("ppo.status in ('PUBLISH','CLOSED')");
        $this->db3->order_by('p.nama_produk','asc');
        $this->db3->group_by('prm.material_id');
    ...
    function GetPenerimaanPembelianSCPrint($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false,$filter_kategori=false)
    {
        $output = array();

        $this->db3->select('prm.purchase_order_id, prm.display_measure as measure,p.nama_produk,prm.material_id,SUM(prm.display_price) / SUM(prm.display_volume) as price,SUM(prm.display_volume) as volume, SUM(prm.display_price) as total_price');
        $this->db3->join('produk p','prm.material_id = p.id','left');
        $this->db3->join('pmm_purchase_order ppo','prm.purchase_order_id = ppo.id','left');
        if(!empty($start_date) && !empty($end_date)){
            $this->db3->where('prm.date_receipt >=',$start_date);
    ...
    function GetLaporanHutangTEMEF($supplier_id=false,$start_date=false,$end_date=false,$filter_kategori=false)
    {
        $output = array();

        $this->db2->select('prm.purchase_order_id, p.nama_produk, SUM(prm.display_price) as penerimaan,
        (
            select SUM(ppd.total) 
            from pmm_penagihan_pembelian_detail ppd 
            inner join pmm_penagihan_pembelian ppp 
		...
		$this->db2->where("ppo.status in ('PUBLISH','CLOSED')");
        $this->db2->order_by('ppo.date_po','asc');
        $this->db2->group_by('prm.purchase_order_id');
        $query = $this->db2->get('pmm_receipt_material prm');
        $output = $query->result_array();
    ...
    function GetLaporanHutangSC($supplier_id=false,$start_date=false,$end_date=false,$filter_kategori=false)
    {
        $output = array();

        $this->db3->select('prm.purchase_order_id, p.nama_produk, SUM(prm.display_price) as penerimaan,
        (
            select SUM(ppd.total) 
            from pmm_penagihan_pembelian_detail ppd 
            inner join pmm_penagihan_pembelian ppp 
		...
		$this->db3->where("ppo.status in ('PUBLISH','CLOSED')");
        $this->db3->order_by('ppo.date_po','asc');
        $this->db3->group_by('prm.purchase_order_id');
        $query = $this->db3->get('pmm_receipt_material prm');
        $output = $query->result_array();
    ...
    function GetLaporanMonitoringPiutangSC($client_id=false,$start_date=false,$end_date=false,$filter_kategori=false,$filter_status=false)
    {
        $output = array();

        $this->db3->select('ppp.*, ps.nama, po.jobs_type as subject,
        (select COALESCE(sum(total),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as dpp_tagihan,
        (select COALESCE(sum(tax),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as ppn_tagihan,
        (select COALESCE(sum(total),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") +  (select COALESCE(sum(tax),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as jumlah_tagihan,
        ...
        if(!empty($start_date) && !empty($end_date)){
            $this->db3->where('ppp.tanggal_invoice >=',$start_date.' 00:00:00');
            $this->db3->where('ppp.tanggal_invoice <=',$end_date.' 23:59:59');
        }
        if(!empty($client_id) || $client_id != 0){
            $this->db3->where('ppp.client_id',$client_id);
        }
        if(!empty($filter_kategori) || $filter_kategori != 0){
            $this->db3->where_in('po.kategori_id',$filter_kategori);
        }
        if(!empty($filter_status) || $filter_status != 0){
            $this->db3->where_in('ppp.status_pembayaran',$filter_status);
        }
        $this->db3->order_by('ppp.tanggal_invoice','asc');
        $this->db3->group_by('ppp.id');
    ...
    function GetLaporanMonitoringHutangTEMEF($supplier_id=false,$start_date=false,$end_date=false,$filter_kategori=false,$filter_status=false)
    {
        $output = array();

        $this->db2->select('ppp.*, pvp.tanggal_lolos_verifikasi, pvp.status_umur_hutang, ps.nama, ppo.subject,
        (select COALESCE(sum(total),0) from pmm_penagihan_pembelian_detail ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as dpp_tagihan,
        (select COALESCE(sum(ppn),0) from pmm_verifikasi_penagihan_pembelian ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as ppn_tagihan,
        (select COALESCE(sum(total),0) from pmm_penagihan_pembelian_detail ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") +  (select COALESCE(sum(ppn),0) from pmm_verifikasi_penagihan_pembelian ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as jumlah_tagihan,
        ...
        if(!empty($start_date) && !empty($end_date)){
            $this->db2->where('ppp.created_on >=',$start_date.' 00:00:00');
        ...
        if(!empty($filter_status) || $filter_status != 0){
            $this->db2->where_in('ppp.status',$filter_status);
        }
        
        $this->db2->where("ppp.verifikasi_dok in ('SUDAH','LENGKAP')");
        $this->db2->order_by('ppp.tanggal_invoice','asc');
        $this->db2->group_by('ppp.id');
    ...
    function GetLaporanMonitoringHutangSC($supplier_id=false,$start_date=false,$end_date=false,$filter_kategori=false,$filter_status=false)
    {
        $output = array();

        $this->db3->select('ppp.*, pvp.tanggal_lolos_verifikasi, pvp.status_umur_hutang, ps.nama, ppo.subject,
        (select COALESCE(sum(total),0) from pmm_penagihan_pembelian_detail ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as dpp_tagihan,
        (select COALESCE(sum(ppn),0) from pmm_verifikasi_penagihan_pembelian ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as ppn_tagihan,
        (select COALESCE(sum(total),0) from pmm_penagihan_pembelian_detail ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") +  (select COALESCE(sum(ppn),0) from pmm_verifikasi_penagihan_pembelian ppd where ppd.penagihan_pembelian_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as jumlah_tagihan,
        ...
        if(!empty($start_date) && !empty($end_date)){
            $this->db3->where('ppp.created_on >=',$start_date.' 00:00:00');
        ...
        if(!empty($filter_status) || $filter_status != 0){
            $this->db3->where_in('ppp.status',$filter_status);
        }
    ...
    function GetDaftarPembayaranTEMEF($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false)
    {
        $output = array();
		
        $this->db2->select('ppp.id as penagihan_id, pmp.tanggal_pembayaran, pmp.nomor_transaksi, ppp.tanggal_invoice, ppp.nomor_invoice, pmp.total as pembayaran');
		$this->db2->join('pmm_penagihan_pembelian ppp', 'pmp.penagihan_pembelian_id = ppp.id');
        
		if(!empty($start_date) && !empty($end_date)){
            $this->db2->where('pmp.tanggal_pembayaran >=',$start_date.' 00:00:00');
            $this->db2->where('pmp.tanggal_pembayaran <=',$end_date.' 23:59:59');
        }
		
		if(!empty($supplier_id)){
            $this->db2->where('pmp.supplier_name',$supplier_id);
        }
        if(!empty($purchase_order_no)){
        ...
        $this->db2->order_by('pmp.tanggal_pembayaran','desc');
    ...
    function GetDaftarPembayaranSC($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false)
    {
        $output = array();
		
        $this->db3->select('ppp.id as penagihan_id, pmp.tanggal_pembayaran, pmp.nomor_transaksi, ppp.tanggal_invoice, ppp.nomor_invoice, pmp.total as pembayaran');
		$this->db3->join('pmm_penagihan_pembelian ppp', 'pmp.penagihan_pembelian_id = ppp.id');
        
		if(!empty($start_date) && !empty($end_date)){
            $this->db3->where('pmp.tanggal_pembayaran >=',$start_date.' 00:00:00');
            $this->db3->where('pmp.tanggal_pembayaran <=',$end_date.' 23:59:59');
        }
		
		if(!empty($supplier_id)){
            $this->db3->where('pmp.supplier_name',$supplier_id);
        }
        if(!empty($purchase_order_no)){
        ...
        $this->db3->order_by('pmp.tanggal_pembayaran','desc');
    ...
    function GetPengirimanPenjualanTEMEF($filter_client_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_product=false)
    {
        $output = array();

        $this->db2->select('pp.salesPo_id, pp.measure, pp.measure, p.nama_produk, pp.display_harga_satuan, SUM(pp.display_volume) as total, SUM(pp.display_price) / SUM(pp.display_volume) as price, SUM(pp.display_price) as total_price');
        $this->db2->join('produk p','pp.product_id = p.id','left');
        ...
        if(!empty($filter_client_id)){
            $this->db2->where('pp.client_id',$filter_client_id);
        }
        if(!empty($purchase_order_no)){
            $this->db2->where('ppo.id',$purchase_order_no);
        }
        ...
        $this->db2->order_by('pp.salesPo_id','asc');
        $this->db2->order_by('p.nama_produk','asc');
        $this->db2->group_by('pp.display_harga_satuan');
    ...
    function GetPengirimanPenjualanTEMEFPrint($filter_client_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_product=false)
    {
        $output = array();

        $this->db2->select('pp.salesPo_id, pp.measure, pp.convert_measure, p.nama_produk, pp.display_harga_satuan, SUM(pp.display_volume) as total, SUM(pp.display_price) / SUM(pp.display_volume) as price, SUM(pp.display_price) as total_price');
        $this->db2->join('produk p','pp.product_id = p.id','left');
        $this->db2->join('pmm_sales_po ppo','pp.salesPo_id = ppo.id','left');
        if(!empty($start_date) && !empty($end_date)){
            $this->db2->where('pp.date_production >=',$start_date);
    ...
    function GetPengirimanPenjualanSC($filter_client_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_product=false)
    {
        $output = array();

        $this->db3->select('pp.salesPo_id, pp.measure, p.nama_produk, SUM(pp.display_volume) as total, SUM(pp.display_price) / SUM(pp.display_volume) as price, SUM(pp.display_price) as total_price');
        $this->db3->join('produk p','pp.product_id = p.id','left');
        ...
        if(!empty($filter_client_id)){
            $this->db3->where('pp.client_id',$filter_client_id);
        }
        if(!empty($purchase_order_no)){
            $this->db3->where('ppo.id',$purchase_order_no);
        }
        ...
        $this->db3->where("pp.product_id in (3,4,7,8,9,14,24,35,36,37,38)");
        $this->db3->order_by('p.nama_produk','asc');
        $this->db3->group_by('pp.product_id');
    ...
    function GetLaporanPiutangTEMEF($client_id=false,$start_date=false,$end_date=false)
    {
        $output = array();

        $this->db2->select('pp.salesPo_id, p.nama_produk, SUM(pp.display_price) as penerimaan,
        (
            select SUM(ppd.total) 
            from pmm_penagihan_penjualan_detail ppd 
            inner join pmm_penagihan_penjualan ppp 
        ...
        $this->db2->order_by('po.contract_date','asc');
        $this->db2->group_by('pp.salesPo_id');
        $query = $this->db2->get('pmm_productions pp');
        $output = $query->result_array();
    ...
    function GetLaporanPiutangSC($client_id=false,$start_date=false,$end_date=false)
    {
        $output = array();

        $this->db3->select('pp.salesPo_id, p.nama_produk, SUM(pp.display_price) as penerimaan,
        (
            select SUM(ppd.total) 
            from pmm_penagihan_penjualan_detail ppd 
            inner join pmm_penagihan_penjualan ppp 
        ...
        $this->db3->order_by('po.contract_date','asc');
        $this->db3->group_by('pp.salesPo_id');
        $query = $this->db3->get('pmm_productions pp');
        $output = $query->result_array();
    ...
    function GetLaporanMonitoringPiutangTEMEF($client_id=false,$start_date=false,$end_date=false,$filter_kategori=false,$filter_status=false)
    {
        $output = array();

        $this->db2->select('ppp.*, ps.nama, po.jobs_type as subject,
        (select COALESCE(sum(total),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as dpp_tagihan,
        (select COALESCE(sum(tax),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as ppn_tagihan,
        (select COALESCE(sum(total),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") +  (select COALESCE(sum(tax),0) from pmm_penagihan_penjualan_detail ppd where ppd.penagihan_id = ppp.id and ppp.tanggal_invoice >= "'.$start_date.'"  and ppp.tanggal_invoice <= "'.$end_date.'") as jumlah_tagihan,
        ...
        if(!empty($start_date) && !empty($end_date)){
            $this->db2->where('ppp.tanggal_invoice >=',$start_date.' 00:00:00');
            $this->db2->where('ppp.tanggal_invoice <=',$end_date.' 23:59:59');
        }
        if(!empty($client_id) || $client_id != 0){
            $this->db2->where('ppp.client_id',$client_id);
        }
        if(!empty($filter_kategori) || $filter_kategori != 0){
            $this->db2->where_in('po.kategori_id',$filter_kategori);
        }
        if(!empty($filter_status) || $filter_status != 0){
            $this->db2->where_in('ppp.status_pembayaran',$filter_status);
        }
        $this->db2->order_by('ppp.tanggal_invoice','asc');
        $this->db2->group_by('ppp.id');
    ...
    function GetDaftarPenerimaanTEMEF($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false)
    {
        $output = array();
		
        $this->db2->select('pmp.client_id, pmp.tanggal_pembayaran, pmp.nomor_transaksi, ppp.tanggal_invoice, ppp.nomor_invoice, pmp.total as penerimaan');
    ...
    function GetDaftarPenerimaanSC($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false)
    {
        $output = array();
		
        $this->db3->select('pmp.client_id, pmp.tanggal_pembayaran, pmp.nomor_transaksi, ppp.tanggal_invoice, ppp.nomor_invoice, pmp.total as penerimaan');
